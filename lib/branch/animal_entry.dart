...
import 'package:fieldhand/widgets/tap_out_detector.dart';
...
class AnimalEntry extends StatefulWidget {
  ...
  AnimalEntry({@required this.edit, @required this.farmId, this.animal});
  ...
  @override
  _AnimalEntryState createState() => _AnimalEntryState(farmId);
}

class _AnimalEntryState extends State<AnimalEntry> {

  _AnimalEntryState(this.farmId);
  final String farmId;
  ...
  void findLinks() async {
    if (animal.sire != null) {
      links['sireId'] = (await queryTableSerial(table: Animal.table, serial: animal.sire)).displayIdentifier;
    }
    if (animal.dam != null) {
      links['damId'] = (await queryTableSerial(table: Animal.table, serial: animal.dam)).displayIdentifier;
    }
    setState(() {});
  }
                  ...
                  function: () {
                    if (saveSafe) {
                      ...
                      if (widget.edit == false) {
                        save(farmId: farmId, objectTable: Animal.table, object: animal);
                      } else {
                        update(objectTable: Animal.table, object: animal);
                      }
                      navigate(context: context,
                          page: Livestock(farmId: farmId),
                          direction: 'right',
                          fromDrawer: false,
                          pop: true);
                    }
